...
namespace
{
  ...
  void adjacentPoints(const int i, const int N, const geometry_msgs::Polygon poly, geometry_msgs::Point32 *p0, geometry_msgs::Point32 *p1, geometry_msgs::Point32 *p2)
  {
  ...
  //朝向
  void laneletDirectionAsMarker(const lanelet::ConstLanelet ll, visualization_msgs::Marker *marker, const int id, const std::string ns)
  {
    if (marker == nullptr)
    {
      ROS_ERROR_STREAM(__FUNCTION__ << ": marker is null pointer!");
...
namespace lanelet
{
      ...
      const lanelet::ConstLanelets &lanelets, const std_msgs::ColorRGBA c, const bool viz_centerline)
  {
    ...
    for (auto li = lanelets.begin(); li != lanelets.end(); li++)
    {
      ...
      if (!exists(added, left_ls.id())) //假如不存在
      {
