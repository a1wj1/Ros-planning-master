...
namespace mission_planner
{
  ...
  void MissionPlannerLanelet2::visualizeRoute(const autoware_planning_msgs::Route &route) const
  {
    ...
    insertMarkerArray(&route_marker_array, lanelet::visualization::laneletsAsTriangleMarkerArray("normal_lanelets", normal_lanelets, cl_normal));
    insertMarkerArray(&route_marker_array, lanelet::visualization::laneletsAsTriangleMarkerArray("goal_lanelets", goal_lanelets, cl_goal));
  ...
  autoware_planning_msgs::Route MissionPlannerLanelet2::planRoute()
  {
    ...
    {
      ss << "xxx: " << checkpoint.pose.position.x << " "
         << "yyy: " << checkpoint.pose.position.y << std::endl;
    }
    ...
    for (std::size_t i = 1; i < checkpoints_.size(); i++)
    {
      ...
      if (!planPathBetweenCheckpoints(start_checkpoint, goal_checkpoint, &path_lanelets)) //返回最短路径
      {
        return route_msg;
      }
      ...
      lanelet::ConstLanelets *path_lanelets_ptr) const
  {
    ...
    lanelet::Optional<lanelet::routing::Route> optional_route = routing_graph_ptr_->getRoute(start_lanelet, goal_lanelet, 0);
    if (!optional_route)
    {
      ROS_ERROR_STREAM(
