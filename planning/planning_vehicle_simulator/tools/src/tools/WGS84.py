...
class UtmVector:
    ...
    def __init__(self, x = 0, y = 0, azimuth = 0):
        self.x = x
        self.y = y
...
class SensorGpsUTM:
    ...
    def __init__(self, x = 0, y = 0, heading = 0, velocity = 0, pitch = 0, roll = 0, status = 1, satenum = 0, timestamp = 0):
        self.x = x
        self.y = y
...
class NewRoadPoint:
    ...
    def __init__(self, lat = 0, lon = 0, heading = 0, speed = 3, obs = 1, follow = 1, special = 1, search = 1, cross = 1, rtk = 1, reserved = 1):
        self.lon = lon
        self.lat = lat
        self.heading = heading
        self.speed = speed
...
class NewRoadPointUTM:
    
    def __init__(self, x = 0, y = 0, heading = 0, speed = 3, obs = 1, follow = 1, special = 1, search = 1, cross = 1, rtk = 1, reserved = 1):
        self.x = x
        self.y = y
        self.heading = heading
        self.speed = speed
...
class WGS84Vector:
    ...
    def __init__(self, lat, lon, azimuth):
        self.lat = lat
        self.lon = lon
        self.azimuth = azimuth
...
class CoordConverter:
    ...
    def TransWgsVectorToUtmVector(self, WGSVector):
        (x, y) = self.LatLonToUTMXY(WGSVector.lat, WGSVector.lon)
        UtmV = UtmVector(x, y, WGSVector.azimuth)
        return UtmV

    
    def TransUtmVectorToWgsVector(self, UtmV1, zone):
        WgsCoorTemp = WGS84Coor()
        (WgsCoorTemp.lat, WgsCoorTemp.lon) = self.UTMXYToLatLon(UtmV1.x, UtmV1.y, zone, False)
        WgsVTemp = WGS84Vector(WgsCoorTemp.lat, WgsCoorTemp.lon, UtmV1.azimuth)
        return WgsVTemp
    ...
    def InterpolateCubic(cls, x0, y0, azimuth0, x1, y1, azimuth1, distance):
        (x_end, y_end, azimuth_end) = cls.get_relative_coord(x0, y0, azimuth0, x1, y1, azimuth1)
        (a, b) = cls.ComputeCubic(x_end, y_end, azimuth_end)
        x = 0
        p_list_local = []
        while x < x_end:
            doty = 3 * a * x ** 2 + 2 * b * x
            theta = np.arctan(doty)
            dx = distance * np.cos(theta)
            x += dx
            y = a * x ** 3 + b * x ** 2
            doty = 3 * a * x ** 2 + 2 * b * x
            theta = np.arctan(doty)
            azi = 90 - cls.RadToDeg(theta)
            if x < x_end:
                p_utmv = UtmVector(x, y, azi)
                p_list_local.append(p_utmv)
                continue
            p_list_global = []
            for p in p_list_local:
                (xt, yt, azimuth_t) = cls.get_global_coord(x0, y0, azimuth0, p.x, p.y, p.azimuth)
                p_list_global.append(np.array([
                    xt,
                    yt,
                    azimuth_t]))
            
        return p_list_global

    InterpolateCubic = classmethod(InterpolateCubic)
    ...
    def MoveRightWGS(self, Point, Distance):
        PointWgsCoor = WGS84Coor(Point.lat, Point.lon)
        MoveAngle = Point.azimuth + 90
        if MoveAngle > 360:
            MoveAngle -= 360
        (lat, lon) = self.MovePointWgsCoor(PointWgsCoor, MoveAngle, Distance)
        PointMoveVector = WGS84Vector(lat, lon, Point.azimuth)
        return PointMoveVector
    ...
    def ArcLengthOfMeridian(cls, phi):
        ...
        result = alpha * (phi + beta * np.sin(2 * phi) + gamma * np.sin(4 * phi) + delta * np.sin(6 * phi) + epsilon * np.sin(8 * phi))
        return result
    ...
    def MapLatLonToXY(cls, phi, lambda1, lambda0):
        ...
        x = N * np.cos(phi) * l + (N / 6) * np.power(np.cos(phi), 3) * coef13 * np.power(l, 3) + (N / 120) * np.power(np.cos(phi), 5) * coef15 * np.power(l, 5) + (N / 5040) * np.power(np.cos(phi), 7) * coef17 * np.power(l, 7)
        y = cls.ArcLengthOfMeridian(phi) + (t / 2) * N * np.power(np.cos(phi), 2) * np.power(l, 2) + (t / 24) * N * np.power(np.cos(phi), 4) * coef14 * np.power(l, 4) + (t / 720) * N * np.power(np.cos(phi), 6) * coef16 * np.power(l, 6) + (t / 40320) * N * np.power(np.cos(phi), 8) * coef18 * np.power(l, 8)
        return (x, y)
    ...
    def FootpointLatitude(cls, y):
        ...
        result = y_ + beta_ * np.sin(2 * y_) + gamma_ * np.sin(4 * y_) + delta_ * np.sin(6 * y_) + epsilon_ * np.sin(8 * y_)
        return result
    ...
    def MapXYToLatLon(cls, x, y, lambda0):
        ...
        lat = cls.RadToDeg(phif + x2frac * x2poly * x * x + x4frac * x4poly * np.power(x, 4) + x6frac * x6poly * np.power(x, 6) + x8frac * x8poly * np.power(x, 8))
        lon = cls.RadToDeg(lambda0 + x1frac * x + x3frac * x3poly * np.power(x, 3) + x5frac * x5poly * np.power(x, 5) + x7frac * x7poly * np.power(x, 7))
        return (lat, lon)
    ...
    MovePointWgsCoor = classmethod(MovePointWgsCoor)
...
class CoordConverter:
    ...
    def rotate_xy(cls, x, y, theta):
        costheta = np.cos(theta)
        sintheta = np.sin(theta)
        RotateArray = np.array([[costheta,-sintheta],[sintheta,costheta]])
        a = np.matmul(RotateArray, np.array([[x],[y]]))
        return (a[(0, 0)], a[(1, 0)])
